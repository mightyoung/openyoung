...
基于 2026 年 AI Agent 最佳实践，新增以下组件：

- **Tool Contract**: 类型化输入/输出验证 (`src/tools/contract.py`)
- **Tracing**: 步骤级 spans (`src/tools/tracing.py`)
- **EvaluationCoordinator**: 评估协调器 (从 YoungAgent 提取)
...
## 9. AI Docker 运行时 (2026)

> 版本: 1.2.0
> 更新日期: 2026-03-08

YoungAgent 内置 **AI Docker 沙箱执行环境**，为 AI Agent 提供安全可控的代码执行能力，参考 E2B、Modal 设计。
...
| 组件 | 文件 | 功能 |
|------|------|------|
| **AISandbox** | `src/runtime/sandbox.py` | 沙箱核心，资源隔离 |
| **SandboxPool** | `src/runtime/pool.py` | 实例池，自动扩缩容 |
| **SecurityManager** | `src/runtime/security.py` | 安全策略，命令验证 |
| **AuditLogger** | `src/runtime/audit.py` | 审计日志 |
...
@dataclass
class SandboxConfig:
    max_cpu_percent: float = 50.0      # CPU 限制
    max_memory_mb: int = 512            # 内存限制 (MB)
    max_execution_time_seconds: int = 300  # 超时时间
    allow_network: bool = False         # 网络访问
```

#### 安全策略

- **命令白名单**: 仅允许预定义命令
- **危险模式检测**: 正则匹配 subprocess、eval、exec 等
- **文件路径验证**: 防止目录遍历攻击
...
class PoolConfig:
    ...
    scale_up_threshold: float = 0.7     # 扩容阈值
    scale_down_threshold: float = 0.3   # 缩容阈值
    auto_scale_enabled: bool = True     # 自动扩缩容
    warm_up_enabled: bool = True        # 预热实例
```
...
agent.enable_sandbox(max_memory_mb=512, max_execution_time_seconds=300)
